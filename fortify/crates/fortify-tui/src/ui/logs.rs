...
fn draw_status_bar(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let row2 = Line::from(vec![
        ...
        Span::styled(
            "SEC:",
            Style::default().fg(security_color).add_modifier(Modifier::BOLD),
        ),
        Span::styled(
            security_label,
            Style::default().fg(security_color).add_modifier(Modifier::BOLD),
...
fn draw_traffic_panel(frame: &mut Frame, app: &App, area: Rect, is_verified: bool) {
    let (title, title_color, events) = if is_verified {
        ...
        let title = match security_level {
            crate::logging::SecurityLevel::Attack | crate::logging::SecurityLevel::Warning => " 🔴 Active Threats ",
            crate::logging::SecurityLevel::Suspicious | crate::logging::SecurityLevel::Elevated => " ⚠ Suspicious Activity ",
            _ => " 👁 Pending Verification ",
        };
        let color = match security_level {
            crate::logging::SecurityLevel::Attack | crate::logging::SecurityLevel::Warning => Color::Red,
            crate::logging::SecurityLevel::Suspicious | crate::logging::SecurityLevel::Elevated => Color::Yellow,
            _ => Color::DarkGray,
        };
    ...
    let block = Block::default()
        ...
        .title_bottom(Line::from(vec![
            Span::styled(
                activity_indicator,
                Style::default().fg(if recent_count > 50 { Color::Red } else if recent_count > 20 { Color::Yellow } else { Color::DarkGray }),
            ),
        ]).right_aligned());
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled("SID      ", Style::default().fg(Color::DarkGray).add_modifier(Modifier::BOLD)),
        Span::styled("MTD ", Style::default().fg(Color::DarkGray).add_modifier(Modifier::BOLD)),
...
fn format_traffic_line(event: &NetworkEvent, path_max: usize, is_verified: bool) -> Line<'static> {
    ...
    let (method_text, method_color, path_color) = if event.is_asset_bundle {
        ("📦".to_string(), Color::Rgb(150, 150, 150), Color::Rgb(150, 150, 150))
    } else {
        let base_color = if is_verified { Color::White } else { Color::Yellow };
        (format!("{:<3}", event.method.label()), event.method.color(), base_color)
    };
    ...
    Line::from(vec![
        ...
        Span::styled(
            ...
            Style::default().fg(if is_verified { Color::Cyan } else { Color::Rgb(255, 165, 0) }),
        ),
        Span::styled(
            format!("{} ", method_text),
