...
fn draw_progress(frame: &mut Frame, current: usize, area: Rect) {
    ...
    let full_length: usize = WIZARD_STEPS
        .iter()
...
fn draw_step_dependencies(frame: &mut Frame, _app: &App, area: Rect) {
    ...
    for result in &results {
        ...
        let req_marker = if result.required {
            " (required)"
        } else {
            " (optional)"
        };
...
fn draw_step_branding(frame: &mut Frame, app: &App, area: Rect) {
    let content = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(
                &app.config.branding.description,
                Style::default().fg(Color::White),
            ),
        ]),
        Line::from(vec![
            Span::raw("  Color: "),
            Span::styled(
                &app.config.branding.primary_color,
    ...
    let content = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(
                ...
                Style::default().fg(if app.config.captcha.enabled {
                    Color::Green
                } else {
                    Color::Red
                }),
...
fn draw_step_network(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let content = vec![
        ...
        Line::from(vec![
            Span::raw("  Tor SOCKS: "),
            Span::styled(
                format!("127.0.0.1:{}", app.config.network.socks_port),
                Style::default().fg(Color::White),
            ),
        ]),
        Line::from(""),
        Line::from(vec![
            Span::raw("  Vanguards: "),
            Span::styled(
                if app.config.network.vanguards_enabled {
                    "Enabled"
                } else {
                    "Disabled"
                },
                Style::default().fg(if app.config.network.vanguards_enabled {
                    Color::Green
                } else {
                    Color::DarkGray
                }),
...
fn draw_step_mirrors(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let mut content = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(
                &prefix_display,
                Style::default().fg(if app.config.vanity.prefix.is_empty() {
                    Color::DarkGray
                } else {
                    Color::Cyan
                }),
    ...
    content.push(Line::from(vec![
        Span::raw("  Safety Net: "),
        Span::styled(
            if app.config.vanity.safety_net_enabled {
                "Enabled"
            } else {
                "Disabled"
            },
            Style::default().fg(if app.config.vanity.safety_net_enabled {
                Color::Green
            } else {
                Color::DarkGray
            }),
    ...
    let content = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(
                format!(
                    "{} active, {} standby",
                    app.config.mirrors.min_mirrors, app.config.mirrors.standby_mirrors
                ),
        ...
        Line::from(vec![
            ...
            Span::styled(
                &vanity_info,
                Style::default().fg(if app.config.vanity.enabled {
                    Color::Magenta
                } else {
                    Color::DarkGray
                }),
