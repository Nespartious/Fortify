...
impl Gate {
    ...
    pub fn create_verification_with_type(
        ...
        match captcha_type {
            CaptchaType::BmpText => {
                let challenge = CaptchaChallenge::generate_with_difficulty(difficulty);
    ...
    pub fn verify_captcha(&self, session_id: &str, solution: &str) -> Result<()> {
        ...
        let is_valid = match &state.captcha_data {
            ...
            Some(CaptchaData::WordUnscramble(c)) => c.verify(solution),
            Some(CaptchaData::ImageRotation(c)) => c.verify(solution),
